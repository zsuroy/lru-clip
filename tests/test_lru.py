    ...
    def test_cleanup_user_clips_within_limit(self, db_session, test_user):
        ...
        for i in range(5):
            clip = Clip(
                ...
                content=f"Content {i}",
                owner_id=test_user.id,
                created_at=datetime.now(timezone.utc) - timedelta(hours=2)  # Old enough to delete
            )
            db_session.add(clip)
    ...
    def test_cleanup_user_clips_over_limit(self, db_session, test_user):
        ...
        for i in range(5):
            clip = Clip(
                ...
                owner_id=test_user.id,
                created_at=datetime.now(timezone.utc) - timedelta(hours=2),  # Old enough to delete
                last_accessed=datetime.now(timezone.utc) - timedelta(hours=i)  # Different access times
            )
            clips.append(clip)
            db_session.add(clip)
    ...
    def test_cleanup_respects_recent_clips(self, db_session, test_user):
        ...
        for i in range(4):
            clip = Clip(
                ...
                content=f"Content {i}",
                owner_id=test_user.id,
                created_at=datetime.now(timezone.utc) - timedelta(minutes=30 if i < 2 else 120),  # First 2 are recent
                last_accessed=datetime.now(timezone.utc) - timedelta(hours=i)
            )
            db_session.add(clip)
